...
@Component({
    selector: 'app-volumeconverter',
    templateUrl: './volumeconverter.component.html',
    styleUrls: ['./volumeconverter.component.scss']
})
/** volumeconverter component*/
export class VolumeconverterComponent {
    /** volumeconverter ctor */
  ...
  onClickSubmit() {
    ...
    this.httpclient.get<string[]>(this.baseUrl + 'api/volume/' + this.coversionForm.controls['conversionvalue'].value + '/' + this.coversionForm.controls['conversionfrom'].value + '/' + this.coversionForm.controls['conversionto'].value).subscribe(result => {
  ...
  cancel() {
    this.router.navigate(['/volume-convert']);
  }
}
